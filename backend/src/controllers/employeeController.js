...
const createAdvance = async (req, res) => {
  try {
    ...
    notifyAdvance(advance.employee?.name || '', advance.amount, '', parseInt(accountId), reason || null).catch(() => {});
...
const createSalary = async (req, res) => {
  try {
    ...
    if (paid) {
      notifySalaryPaid(salary.employee?.name || '', salary.netSalary, '', parseInt(accountId), salary.note || null).catch(() => {});
    }
